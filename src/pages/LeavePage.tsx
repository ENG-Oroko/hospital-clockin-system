...
import { leaveRecordsData, departmentsData } from '../data'
...
const LeavePage: React.FC = () => {
  ...
  const countByDept = departmentsData.map(d => ({
    name:  d.name,
    color: d.color,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
        <div
          style={{ background: '#fff', border: `2px solid ${selectedDept === 'all' ? '#2563eb' : '#e5e7eb'}`, borderRadius: 12, padding: '14px 18px', cursor: 'pointer' }}
        ...
        {countByDept.filter(d => d.count > 0).map(d => (
          <div
            key={d.name}
            style={{ background: '#fff', border: `2px solid ${selectedDept === d.name ? d.color : '#e5e7eb'}`, borderRadius: 12, padding: '14px 18px', cursor: 'pointer', transition: 'border .15s' }}
